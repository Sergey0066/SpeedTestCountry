...
def main():
    ...
    if args.region:
        choice = args.region.lower()
    elif interactive:
        choice = prompt_choice().lower()
    else:
        console.print("[yellow]Нет консоли ввода.")
        choice = "all"
    console.print()
    ...
    if choice == "custom":
        if args.url:
            ...
            url = Prompt.ask("Вставь URL для скачивания (файл)")
        else:
            raise SystemExit("Для режима custom нужно указать URL: --url https://example.com/file.bin")
